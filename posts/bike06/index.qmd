...
title: bike06    # HIER TITEL DES POSTS EINGEBEN.
execute: 
  eval: true 
  cache: true
...
A) Was ist der *Punktschätzer* des Effekts der UV?
B) Was ist die *Modellgüte* als Anteil der erklärten Varianz des Modells (durch die UV)?
...
```{r}
library(tidyverse)
day <- read_csv(paste0(here::here(), "/data/bike+sharing+dataset/day.csv"))  # Achtung: Das ist der Pfad auf meinem Computer. Passen Sie ggf. den Pfad an Ihren Computer an.
```

...
```{r}
flights_path <- "https://raw.githubusercontent.com/sebastiansauer/Lehre/main/data/flights_small.csv"
flights <- read.csv(flights_path)
```
...
flights_train6_pred <- 
  flights_train6 %>% 
  mutate(lm6_pred = predict(lm6, newdata = flights_train6))  
```
...
Ein kurze Schätzung der Rechenzeit in Abhängikeit der Anzahl der Prädiktoren bei der Best-Subset-Methode; gehen wir von 1, 2 oder 5 Sekunden Rechenzeit (`exec_time`) pro Modell aus.
...
# VERTIEFUNG lm8: Bestes Modell aus der Best-Subset-Analyse
